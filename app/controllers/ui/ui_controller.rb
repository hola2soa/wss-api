...
module Show
  def self.registered(app)
    ...
    @item = params[:item]
      ...
      #combine result
      @products=[]
      @results1.each {|result| @products.push(result) if result["price"].to_i>0} if @results1!=nil
      @results2.each {|result| @products.push(result) if result["price"].to_i>0} if @results1!=nil
      @results3.each {|result| @products.push(result) if result["price"].to_i>0} if @results1!=nil
    ...
    app_get_show_item = lambda do
      begin
        ...
        if session[:results]
          ...
          session[:results] = nil
        elsif @item == 'pants' || @item == 'popular' || @item == 'accessories' || @item == 'tops' || @item == 'latest'  #category
          request_url = get_api_url("?store=queenshop&category=#{@item}")
          @results1 = HTTParty.get(request_url, options)
          request_url = get_api_url("?store=stylemooncat&category=#{@item}")
          @results2 = HTTParty.get(request_url, options)
          request_url = get_api_url("?store=joyceshop&category=#{@item}")
          @results3 = HTTParty.get(request_url, options)
        else #keyword
        ...
        #combine results
        @results1.each { |result| @products.push(result) if result["price"].to_i>0} if @results1!=nil
        @results2.each { |result| @products.push(result) if result["price"].to_i>0} if @results2!=nil
        @results3.each { |result| @products.push(result) if result["price"].to_i>0} if @results3!=nil
